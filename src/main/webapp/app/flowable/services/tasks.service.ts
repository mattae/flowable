...
import { CompleteForm, FormModel, ListResult, SaveForm } from '../model/common.model';

@Injectable({
    providedIn: 'root'
})
export class TasksService {
    private resourceUrl = '/app/rest/tasks';
    ...
    involveTask(taskId: string, userId: string) {
        return this.http.put<TaskInstance>(`${this.resourceUrl}/${taskId}/action/involve`, {userId: userId}).pipe(
            map(res => this.mapTaskFromServer(res))
        );
    }
    ...
    getTaskForm(taskId: string) {
        return this.http.get<FormModel>(`/app/rest/task-forms/${taskId}`);
    }

    completeTaskForm(taskId: string, payload: CompleteForm) {
        return this.http.post<void>(`/app/rest/task-forms/${taskId}`, payload);
    }

    saveTaskForm(taskId: string, payload: SaveForm) {
        return this.http.post<void>(`/app/rest/task-forms/${taskId}/save-form`, payload);
    }

    listTasks(taskId: string, payload: {
        state?: string;
        ...
        return this.http.post<ListResult<TaskInstance>>(`/app/rest/query/tasks`, payload).pipe(
            map(res => {
