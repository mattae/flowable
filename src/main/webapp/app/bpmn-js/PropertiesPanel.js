...
var domify = require('min-dom').domify, domQuery = require('min-dom').query, domQueryAll = require('min-dom').queryAll,
    domRemove = require('min-dom').remove, domClasses = require('min-dom').classes,
    domClosest = require('min-dom').closest, domAttr = require('min-dom').attr,
    domDelegate = require('min-dom').delegate, domMatches = require('min-dom').matches;

var forEach = require('lodash/forEach'), filter = require('lodash/filter'), get = require('lodash/get'),
    keys = require('lodash/keys'), isEmpty = require('lodash/isEmpty'), isArray = require('lodash/isArray'),
    xor = require('lodash/xor'), debounce = require('lodash/debounce');
...
PropertiesPanel.prototype.activateTab = function (tab) {
    ...
    forEach(allTabLinkNodes, function (tabLinkNode) {
        var tabLink = domQuery('[data-tab-target]', tabLinkNode), currentTabId = domAttr(tabLink, 'data-tab-target');
...
PropertiesPanel.prototype._entriesChanged = function (current, newTabs) {
    var oldEntryIds = keys(current.entries), newEntryIds = keys(extractEntries(newTabs));
...
PropertiesPanel.prototype._bindListeners = function (container) {
    ...
    var handleChange = function handleChange(event) {
        ...
        // if not, drop out
        var inputNode = event.delegateTarget, entryNode = domClosest(inputNode, '[data-entry]'), entryId, entry;
    ...
    domDelegate.bind(container, '[data-action]', 'click', function onClick(event) {
        ...
        var inputNode = event.delegateTarget, entryNode = domClosest(inputNode, '[data-entry]');

        var actionId = domAttr(inputNode, 'data-action'), entryId = domAttr(entryNode, 'data-entry');
    ...
    function handleInput(event, element) {
        ...
        var eventHandlerId = domAttr(inputNode, 'data-blur'), entryId = domAttr(entryNode, 'data-entry');
...
PropertiesPanel.prototype.applyChanges = function (entry, values, containerElement) {
    ...
    if (isArray(command)) {
        if (command.length) {
            commandToExecute = {
                cmd: 'properties-panel.multi-command-executor', context: flattenDeep(command),
            };
        }
    } else {
...
var flattenDeep = require('lodash/flattenDeep'), keyBy = require('lodash/keyBy'), map = require('lodash/map');
...
PropertiesPanel.prototype._create = function (element, tabs) {
    ...
    return {
        tabs: tabs, groups: groups, entries: entries, element: element, panel: panelNode,
    };
};
...
PropertiesPanel.prototype._bindTemplate = function (element, entry, values, entryNode, idx) {
    ...
    function isPropertyEditable(entry, propertyName) {
        return eventBus.fire('propertiesPanel.isPropertyEditable', {
            entry: entry, propertyName: propertyName, element: element,
        });
    }
...
PropertiesPanel.prototype._createPanel = function (element, tabs) {
    ...
    forEach(tabs, function (tab, tabIndex) {
        ...
        forEach(groups, function (group) {
            ...
            var groupNode = domify('<div class="bpp-properties-group" data-group="' + escapeHTML(group.id) + '">' + '<span class="group-toggle"></span>' + '<span class="group-label">' + escapeHTML(group.label) + '</span>' + '</div>',);
...
function getContentEditableSelection(node) {
    ...
    return {
        start: Math.min(focusOffset, anchorOffset), end: Math.max(focusOffset, anchorOffset),
    };
}
