...
function getFormControlValuesInScope(entryNode) {
    ...
    forEach(controlNodes, function (controlNode) {
        ...
        // 下拉框多选值设置
        if (controlNode.multiple) {
            let tempValues = '';
            const selectedOptions = controlNode.selectedOptions;
            forEach(selectedOptions, function (selectedOption) {
                tempValues += selectedOption.value + ',';
            });
            value = tempValues.substring(0, tempValues.length - 1);
        }
 ...
 */
export default function FlowablePropertiesPanel(config, eventBus, modeling, propertiesProvider, commandStack, canvas) {
...
FlowablePropertiesPanel.$inject = [
    'propertiesPanel',
    'eventBus',
    'modeling',
    'propertiesProvider',
...
FlowablePropertiesPanel.prototype._init = function (config) {
...
FlowablePropertiesPanel.prototype.attachTo = function (parentNode) {
    if (!parentNode) {
...
FlowablePropertiesPanel.prototype.detach = function () {
    var container = this._container,
 ...
 */
FlowablePropertiesPanel.prototype.activateTab = function (tab) {
 ...
 */
FlowablePropertiesPanel.prototype.update = function (element) {
    var current = this._current;
 ...
 * @return {Boolean}
 */
FlowablePropertiesPanel.prototype._entriesChanged = function (current, newTabs) {
...
FlowablePropertiesPanel.prototype._emit = function (event) {
    this._eventBus.fire('propertiesPanel.' + event, {panel: this, current: this._current});
};

FlowablePropertiesPanel.prototype._bindListeners = function (container) {
    var self = this;
    ...
    domDelegate.bind(container, '.bpp-properties-tabs-links [data-tab-target]', 'click', function (
        event,
    ) {
        event.preventDefault();
...
FlowablePropertiesPanel.prototype.updateState = function (entry, entryNode) {
 ...
 */
FlowablePropertiesPanel.prototype.updateShow = function (entry, node) {
    var current = this._current;
 ...
 */
FlowablePropertiesPanel.prototype.updateDisable = function (entry, node) {
    var current = this._current;
...
FlowablePropertiesPanel.prototype.executeAction = function (entry, entryNode, actionId, event) {
    var current = this._current;
 ...
 */
FlowablePropertiesPanel.prototype.applyChanges = function (entry, values, containerElement) {
    ...
    // 解决validate验证不通过，但依然将属性值插入xml中的问题
    // var current = this._current;
    // const entryNode = domQuery('[data-entry="' + entry.id + '"]', current.panel);
    // const validateResult = this.validate(entry, values, entryNode);
    // if(validateResult) {
    //   if (commandToExecute) {
    ...
    if (commandToExecute) {
        this._commandStack.execute(
            ...
            commandToExecute.context || {element: element},
        );
    } else {
        this.update(element);
    }
 ...
 */
FlowablePropertiesPanel.prototype.applyValidationErrors = function (validationErrors, entryNode) {
    var valid = true;
 ...
 */
FlowablePropertiesPanel.prototype.validate = function (entry, values, entryNode) {
    var self = this;
...
FlowablePropertiesPanel.prototype.getEntry = function (id) {
...
FlowablePropertiesPanel.prototype._create = function (element, tabs) {
    if (!element) {
        return null;
    }
 ...
 * @param {Number} idx
 */
FlowablePropertiesPanel.prototype._bindTemplate = function (element, entry, values, entryNode, idx) {
...
// 在这里设置哪个element显示那些tab
FlowablePropertiesPanel.prototype._updateActivation = function (current) {
    var self = this;
...
FlowablePropertiesPanel.prototype._createPanel = function (element, tabs) {
    var self = this;
        ...
        forEach(groups, function (group) {
            ...
            forEach(group.entries, function (entry) {
                ...
                var entryNode = domify(
                    '<div class="bpp-properties-entry" data-entry="' + escapeHTML(entry.id) + '"></div>',
                );
