...
package com.mattae.snl.plugins.flowable.content.spring;
...
public class SpringContentEngineConfiguration extends ContentEngineConfiguration implements SpringEngineConfiguration {
    ...
    public CommandInterceptor createTransactionInterceptor() {
        if (transactionManager == null) {
            throw new FlowableException("transactionManager is required property for SpringFormEngineConfiguration, use " + getClass().getName() + " otherwise");
        }
    ...
    public ContentEngineConfiguration setDataSource(DataSource dataSource) {
        if (dataSource instanceof TransactionAwareDataSourceProxy) {
            return (ContentEngineConfiguration) super.setDataSource(dataSource);
        } else {
            // Wrap datasource in Transaction-aware proxy
            DataSource proxiedDataSource = new TransactionAwareDataSourceProxy(dataSource);
            return (ContentEngineConfiguration) super.setDataSource(proxiedDataSource);
        }
    }
